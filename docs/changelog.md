...
- API: remove `GET /api/security/repos` and `GET /api/security/repos/{owner}/{repo}` (plain **404**). Repo score rollups still update after finished runs; there is no public scoreboard HTTP surface.
