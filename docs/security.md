...
- A route that a peer calls must be registered with `registerPeerAuth` (user **or** service JWT). Routes registered with `authView` / `authAdmin` accept user JWTs only and reject every service JWT with 401 `invalid token`, regardless of whether the secret matches. See `docs/api.md` → Peer-callable AppSec routes.
