...
- Enforce `X-Organization-ID` / `X-Project-ID` tenancy on control-plane routes. When `OPA_AUTH_REQUIRED=1`, omitting those headers (or sending `"all"`) scopes ClickHouse lists to `default-org` / `default-project` (HTTP 200; Open-Tenant-Go ≥ 0.2.2).
